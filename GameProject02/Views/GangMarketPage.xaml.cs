...
public partial class GangMarketPage : ContentPage
{
    ...
    public GangMarketPage(int categoryId, int subCategoryId, string categoryName, string subCategoryName)
    {
        InitializeComponent();
        ...
        if (!string.IsNullOrEmpty(subCategoryName))
            CategoryTitleLabel.Text = $"{categoryName} - {subCategoryName}";
        else
    ...
    private void LoadItems()
    {
        ...
        if (_categoryId == 0 && _subCategoryId >= 0)
            ...
            _items = GangMarketService.GetItemsByCategory(_categoryId);
