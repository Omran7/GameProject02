...
public class IdlibCityMapView : ContentView
{
    ...
    private void InitializeMap()
    {
        Task.Run(async () =>
        {
            try
            {
                // ── Load scene JSON ──────────────────────────────────────────
                string json = "";
                foreach (var file in new[] { "IdlibCityScene.dt" })
                {
    ...
    private async Task<bool> TryLoadBitmap(string file, string cacheKey, string ext)
    {
        try
        {
            using var stream = await FileSystem.OpenAppPackageFileAsync($"Assets/images/IdlibCity/{file}{ext}");
