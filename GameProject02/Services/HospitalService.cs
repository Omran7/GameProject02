    ...
    public static class HospitalService
    {
        ...
        public static (bool success, string message) PerformWork(PlayerAccount player, int workLevel)
        {
            ...
            NotificationService.AddGameNotification(
                "🏥 عمل في المستشفى",
                $"أكملت العمل واستعدت +{healthGained} صحة و +{work.ExperienceReward} خبرة",
                GameNotificationPriority.Normal, "🏥", "HospitalPage"
            );
            ...
            while (player.MainStatesObject.CanLevelUp())
            {
                ...
                NotificationService.AddGameNotification(
                    ...
                    $"تهانينا! وصلت للمستوى {player.Level}\n+{player.Level * 50} ذهب مكافأة",
