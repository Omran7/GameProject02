...
{
    private const int ESCAPE_COURAGE_COST = 5;   // ← من 10 إلى 5
    private const int SMUGGLE_COURAGE_COST = 5;   // ← من 10 إلى 5
    ...
    public static (bool success, string message) AttemptJailbreak(PlayerAccount player)
    {
        ...
        bool success = new Random().Next(100) < 50;  // 50% نجاح
        ...
        {
            long remainingTime = player.CrimeObject.PrisonReleaseTime
    ...
    // ── دفع الكفالة عن لاعب آخر ──────────────────────────────────────────────
    public static (bool success, string message) PayBailForPlayer(
        PlayerAccount currentPlayer, PlayerAccount targetPlayer)
    {
    ...
    // ── محاولة تهريب لاعب آخر ────────────────────────────────────────────────
    public static (bool success, string message) AttemptJailbreakForPlayer(
        PlayerAccount currentPlayer, PlayerAccount targetPlayer)
    {
        if (!targetPlayer.CrimeObject.IsInPrison)
            return (false, "هذا اللاعب ليس في السجن حالياً!");
